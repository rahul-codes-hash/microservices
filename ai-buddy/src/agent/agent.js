const { StateGraph , MessageAnnotation} = require('@langchain/langgraph')
...
const graph = new StateGraph(MessageAnnotation)
.addNode("tools", async ( state , config ) => {
    ...
    const toolCallResults = await Promise.all(toolsCall.map( async ( call ) => {
        ...
        console.log("Invoking tool:", call.name, "with input:", call, "and token:", config.metadata.token)
